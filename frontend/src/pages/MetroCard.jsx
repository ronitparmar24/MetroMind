...
export default function MetroCard() {
  ...
  return (
    <div className="page" style={{ maxWidth: '760px', animation: 'fadeInUp 0.4s ease', fontFamily: "'Inter',system-ui,sans-serif" }}>
      <style>{`
      ...
      <div style={{ marginBottom: '24px' }}>
        <h1 style={{ fontSize: '1.6rem', fontWeight: 800, color: 'var(--text-primary)', margin: 0 }}>Metro Card 💳</h1>
        ...
        <>
          ...
          >
            <div style={{
              ...
              {/* Chip graphic */}
              <div style={{ position:'absolute', right:'28px', top:'28px', width:'44px', height:'34px', borderRadius:'6px', background:'linear-gradient(135deg,#fbbf24,#f59e0b)', boxShadow:'0 2px 8px rgba(251,191,36,0.4)', display:'flex', alignItems:'center', justifyContent:'center' }}>
              ...
              {/* Card number */}
              <div style={{ fontFamily: 'monospace', fontSize: '1rem', fontWeight: 600, color: 'rgba(255,255,255,0.9)', letterSpacing: '2.5px', marginBottom: '20px' }}>
                {card.cardNumber.replace('MC-', '').match(/.{1,4}/g)?.join(' ') || card.cardNumber}
              </div>

              {/* Bottom section — clean flex column, no overlaps */}
          ...
          <div style={{ display: 'flex', gap: '12px', marginBottom: '20px', flexWrap: 'wrap' }}>
            ...
            <StatPill icon="💳" label="Wallet Balance" value={`₹${wallet?.balance ?? '—'}`} color="#f59e0b" />
          ...
          <div style={{
            ...
            <div style={{ fontWeight: 700, fontSize: '0.95rem', color: 'var(--text-primary)', marginBottom: '4px' }}>Top Up Metro Card</div>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '14px' }}>
              {QUICK_AMOUNTS.map(a => (
                <button
                  ...
                  disabled={topUpLoading || (wallet?.balance ?? 0) < a}
                  style={{
                    padding: '8px 16px', borderRadius: '10px', border: '1px solid var(--border-color)',
                    background: (wallet?.balance ?? 0) >= a ? 'var(--bg-tertiary)' : 'var(--bg-tertiary)',
                    color: (wallet?.balance ?? 0) >= a ? 'var(--text-primary)' : 'var(--text-muted)',
                    fontWeight: 700, fontSize: '0.85rem', cursor: (wallet?.balance ?? 0) >= a ? 'pointer' : 'not-allowed',
                    opacity: (wallet?.balance ?? 0) >= a ? 1 : 0.4,
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: topUpAmount ? 'linear-gradient(135deg,#6366f1,#a855f7)' : 'var(--bg-tertiary)',
                  color: topUpAmount ? 'white' : 'var(--text-muted)',
                  fontWeight: 700, fontSize: '0.9rem', cursor: topUpAmount ? 'pointer' : 'not-allowed',
                  transition: 'all 0.15s ease',
                }}
              >
                {topUpLoading ? '…' : 'Add'}
          ...
          {/* ══ BENEFITS ══ */}
          <div style={{ borderRadius: '20px', overflow: 'hidden', border: '1px solid var(--border-color)', background: 'var(--bg-secondary)' }}>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border-color)', fontWeight: 700, fontSize: '0.85rem', color: 'var(--text-primary)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
              Card Benefits
            </div>
